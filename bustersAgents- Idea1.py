...
class BustersAgent(object):
    ...
    def getState(self, gameState):     
        ...
        if element2!=None:
            ...
            if element2 <9 and element2 >= 2:
                element2 = 1
            
            elif element2>=9: #very far away
                element2 = 2
        else:
            element2=2
        #east,west or none
        ...
        #east, west or none(better vertically)
        horiz=1 
        if abs(gx - px) > abs(gy - py):
            horiz = 2
        elif gx>px: #ghost is right
            horiz = 0  #east
        elif gx<px: #ghost-left
            horiz= 1  #west
            
        #north ,south or none (more worthy horizontally)
        vert=1
        if abs(gx - px) < abs(gy - py): #more worthy horizontally
            vert= 2
        elif gy>py: #ghost is above
            vert= 0  #north
        elif gy<py: #ghost is below
            vert= 1  #south
                
        state = [element1, element2, horiz, vert ]
...
class QLearningAgent(BustersAgent):
    ...
    def computePosition(self,state):
        ...
        """
        row = state[0] + 27 * state[1] + 9 *state[2]  + 3*state[3]
    ...
    def getReward(self, state, action, nextstate):
        ...
        reward = 0
        print(state, "state")
        print(nextstate, "next")
       
        if state[0] == 0 or state[1]==0: #you are almost eating the ghost
            reward = 1
