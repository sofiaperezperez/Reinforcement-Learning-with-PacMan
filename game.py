        ...
        while not self.gameOver:
            ...
            if self.catchExceptions:
                try:
                    ...
                    self.estado = agent.getState(self.state.deepCopy())
                    print(self.estado, 44444444)
                except Exception as data:
                    self.mute(agentIndex)
                    self._agentCrash(agentIndex)
                    self.unmute()
            ...
            if agentIndex == 0:
                if not first:
                    agent.update( previous, action, agent.getState(self.state.deepCopy()),  agent.getReward( previous, action, agent.getState(self.state.deepCopy())))                
                previous = agent.getState(observation)
                first = False
