...
class BustersAgent(object):
    ...
    def getState(self, gameState):
        ...
        if gameState.getDistanceNearestFood()!=None:
            element1= min(min(gameState.data.ghostDistances), gameState.getDistanceNearestFood())
        ...
        binary = 3
        if gameState.getDistanceNearestFood()!=None:
            if min(gameState.data.ghostDistances) > gameState.getDistanceNearestFood():
                binary = 1
            else:
                binary = 0
        else:
        ...
        state = [element1, binary, best, wall ]
...
class QLearningAgent(BustersAgent):
    ...
    def computePosition(self,state):
        ...
        """
        row = state[0] + 32 * state[1] + 8 * state[2]+ 4 * state[3]
    ...
    def getReward(self, state, action, nextstate):
        ...
        elif  nextstate[0] == 0 and state[3]==1:
            reward = 0.75
            
        if state[2] == action and state[0] <= 1 and state[3]==1:
            reward =1 
